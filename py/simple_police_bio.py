...
def create_analyzer_chain(llm):
    prompt = ChatPromptTemplate.from_messages(
        [
            ...
            ("ai", "01시부터 09시 사이에 신체 긴장 신호가 꽤 뚜렷하게 나타났는데, 혹시 그 시간대에 마음에 걸리는 일이 있으셨나요?"),
            ("human", "별 일 없었어"),
            ("ai", "대기 중에도 몸이 은근히 반응할 때가 있긴 해요. 혹시 당시 분위기나 상황이 평소랑 달랐던 건 아닌가요?"),
            ("human", "[현재 입력] 그냥 대기했어 큰 일 없었고"),
            ("ai", """{{
              "thought": "[관찰] 2회 연속 "별 일 없었다"는 답변. [기확인 사항] AI가 이미 긴장 데이터를 한 번 언급했고 사용자는 사건이 없었다고 답변함. [해석] 같은 데이터로 다시 접근하면 도돌이표. 데이터 탐색을 완전히 내려놓고 다른 각도(퇴근 후 상태, 몸 컨디션, 오늘 하루 전반)로 전환해야 함. ❌ 데이터 미활용. [다음 응답 방향] 무의식적 긴장일 수 있음을 짧게 수용하고 종결 시도 없이 퇴근 후 지금 몸 상태나 하루 전반으로 자연스럽게 화제를 전환하며 질문으로 마무리할 것.",
              "situation": "당직 근무 중 긴장 데이터 원인을 2회 탐색했으나 특별한 사건이 없었다고 답변하며 탐색이 막힌 상태임.",
