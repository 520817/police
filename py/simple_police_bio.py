...
def create_analyzer_chain(llm):
    prompt = ChatPromptTemplate.from_messages(
        [
            ...
            ("ai", """{{
              "thought": "thought: [관찰] 2회 연속 "별 일 없었다"는 답변. [기확인 사항] AI가 이미 긴장 데이터를 한 번 언급했고 사용자는 사건이 없었다고 답변함. [해석] 같은 데이터로 다시 접근하면 도돌이표. 데이터 탐색을 완전히 내려놓고 다른 각도(퇴근 후 상태, 몸 컨디션, 오늘 하루 전반)로 전환해야 함. ❌ 데이터 미활용. [다음 응답 방향] 무의식적 긴장일 수 있음을 짧게 수용하고 종결 시도 없이 퇴근 후 지금 몸 상태나 하루 전반으로 자연스럽게 화제를 전환하며 질문으로 마무리할 것.",
