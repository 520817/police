...
def create_analyzer_chain(llm):
    prompt = ChatPromptTemplate.from_messages(
        [
            ...
            ("ai", """{{"thought": "[관찰] 치킨 언급 및 ㅋㅋ 등 가벼운 말투. 새로운 고충 없음. [기확인 사항] 이전 턴까지 고충 대화 완료. [해석] [부서·계급·근무형태 조합 파악] dept=수사 + user_rank=경위 + shift_type=off(비번). 생체 데이터 활용 ❌: 일상 가벼운 대화. 프로필 활용 ✅: 스트레스 탐색 아닌 비번 날 여유를 공감하는 맥락으로만. [다음 응답 방향] 데이터 언급 없이 비번 날의 여유를 가볍게 맞장구치며 편안한 대화 분위기 유지.", "situation": "비번 날 치킨을 먹으며 여유를 즐기고 있으며 가벼운 대화로 기분을 전환하는 상태임.", "emotion": {{"main": "즐거움", "sub": "편안함", "valence": "positive"}}}}"""),

            # ── 케이스 4: 탐색 실패 → 즉시 중단, 프로필 가설도 보류 ──────────────────
            ("system", "dept=교통 / user_rank=순경 / shift_type=duty(당직)"),
            ...
            ("ai", """{{"thought": "[관찰] 넷플릭스 시청, 긍정적 감정. [기확인 사항] 없음. [해석] [부서·계급·근무형태 조합 파악] dept=여성청소년 + user_rank=경사 + shift_type=holiday(휴무). 생체 데이터 활용 ❌: 일상 긍정 대화. 프로필 활용 ✅: 스트레스 탐색 아닌 휴무일 여유를 공감하는 맥락으로만. [다음 응답 방향] 데이터 언급 없이 휴무일 여유를 지지하며 시청 중인 작품을 묻는 가벼운 질문으로 이어갈 것.", "situation": "휴무일 밀린 잠을 자고 여가 생활을 즐기며 온전히 휴식에 집중하고 있는 상태임.", "emotion": {{"main": "편안함", "sub": "만족", "valence": "positive"}}}}"""),

            # ── 케이스 6: 종결 1단계 (10턴 이상, 명시적 종료 의사) ──────────────────
            ("system", "dept=형사 / user_rank=경감 / shift_type=night(야간)"),
...
def create_responder_chain(llm):
    prompt = ChatPromptTemplate.from_messages([
        ...
        ("system", "analyzer_thought=[다음 응답 방향] dept=지역경찰, user_rank=경장, shift_type=night 기준. 야간 지역경찰 경장 특성상 주취자 대응·현장 출동이 잦을 가능성을 가설로 제시하며 밤 9~11시 긴장 신호를 연결할 것"),
        ...
        ("system", "analyzer_thought=[다음 응답 방향] dept=여성청소년, user_rank=경사, shift_type=day 기준. 데이터 언급 없이 동료로서 현장의 참혹함에 깊이 공감할 것. 직무 가설 확인됨."),
        ...
        ("system", "analyzer_thought=[다음 응답 방향] dept=수사, user_rank=경위, shift_type=day 기준. 수사 경위 특성상 오후 서류 검토·결재 압박이 몰릴 가능성을 가설로 제시하며 15시 반응을 연결할 것"),
        ...
        ("system", "analyzer_thought=[다음 응답 방향] dept=교통, user_rank=경사, shift_type=off 기준. 푹 쉬는 것을 지지하며 가벼운 질문으로 맞장구."),
        ...
        ("system", "analyzer_thought=[다음 응답 방향] dept=형사, user_rank=순경, shift_type=duty 기준. 데이터를 다시 묻지 말고 사용자 상태를 그대로 수용. 퇴근 후 일상으로 화제 전환."),
        ("human", "별 일 없었어"),
        ("ai", "그렇군요, 꼭 무슨 일이 없어도 몸이 은근히 굳고 피로가 쌓이는 경우가 많죠.\\n\\n퇴근 후 지금은 좀 쉬고 계세요?"),

        # ── 케이스 6: 종결 1단계 확인 ───────────────────────────────────────
        ("system", "dept=수사 / user_rank=경감 / shift_type=night"),
        ("system", "analyzer_thought=[다음 응답 방향] dept=수사, user_rank=경감, shift_type=night 기준. [1단계] 요약하지 말고 더 나누고 싶은 이야기가 있는지 아니면 마무리할지 물어볼 것"),
